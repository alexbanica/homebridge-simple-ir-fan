...
function fixture() {
  ...
  writeFileSync(join(root, 'package.json'), JSON.stringify({ name: 'homebridge-simple-ir-fan', version: '1.0.0' }));
  writeFileSync(
    ...
    JSON.stringify({
      ...
      version: '1.0.0',
      lockfileVersion: 3,
      packages: { '': { name: 'homebridge-simple-ir-fan', version: '1.0.0' } },
    }),
  );
...
fs.appendFileSync(process.env.NPM_CALLS, JSON.stringify({ args, token: process.env.NODE_AUTH_TOKEN ?? null }) + '\\n');
...
if (args[0] === 'pack') {
  ...
  const filename = 'homebridge-simple-ir-fan-' + version + '.tgz';
  ...
  if (process.env.PACK_SHAPE === 'object') {
    const payload = {
      'homebridge-simple-ir-fan': [entry],
    };
...
if (args[0] === 'view') {
  ...
  const latest = process.env.VIEW_LATEST || version;
  const tarball =
    process.env.VIEW_TARBALL ||
    'https://forgejo.alexlab.nl/api/packages/public/npm/homebridge-simple-ir-fan/-/homebridge-simple-ir-fan-' +
      version +
      '.tgz';
  const name = process.env.VIEW_NAME || 'homebridge-simple-ir-fan';
...
function tarballPath(root: string, version: string) {
  return join(root, `homebridge-simple-ir-fan-${version}.tgz`);
}

function readCalls(path: string) {
  return readFileSync(path, 'utf8').trim().split('\n').filter(Boolean).map((line) => JSON.parse(line) as { args: string[]; token: string | null });
}

function run(root: string, env: Record<string, string | undefined>): Invocation {
  const result = spawnSync(process.execPath, [command], {
    cwd: root,
    env: { ...process.env, GIT_CALLS: join(root, 'git-calls.log'), ...env },
    encoding: 'utf8',
  });
...
test('rejects every non-stable tag class named by the release contract', () => {
  for (const tag of ['1.0.0', 'release-v1.0.0', 'v01.0.0', 'v1.0', 'v1.0.0-beta.1', 'v1.0.0+build.1']) {
    const view = fixture();
...
test('requires authentication before release work begins', () => {
  const view = fixture();
  const result = run(view.root, { RELEASE_TAG: 'v1.2.3', PATH: `${view.fakeBin}:${process.env.PATH}`, NPM_CALLS: view.calls });
  assert.notEqual(result.status, 0);
...
test('derives the exact version, aligns package and lockfile, and scopes the token to publish', () => {
  ...
  assert.ok(calls.some((call) => call.args[0] === 'publish' && call.token === 'secret-token'));
...
test('accepts the stable tag examples and removes exactly one leading v', () => {
  for (const [tag, version] of [['v0.1.0', '0.1.0'], ['v1.0.0', '1.0.0'], ['v12.34.56', '12.34.56']]) {
    const view = fixture();
    const result = run(view.root, {
      RELEASE_TAG: tag,
      ...
      VIEW_LATEST: version,
      VIEW_TARBALL: `https://forgejo.alexlab.nl/api/packages/public/npm/homebridge-simple-ir-fan/-/homebridge-simple-ir-fan-${version}.tgz`,
      PATH: `${view.fakeBin}:${process.env.PATH}`,
      NPM_CALLS: view.calls,
    });
...
test('rejects whitespace and extra-suffix tag variants before invoking npm', () => {
  for (const tag of [' v1.0.0', 'v1.0.0 ', 'v1.0.0\n', 'v1.0.0-extra', 'v1.0.0/foo', 'vv1.0.0']) {
    const view = fixture();
    const result = run(view.root, {
      RELEASE_TAG: tag,
      NODE_AUTH_TOKEN: 'secret',
...
test('selects the matching package from machine-readable npm pack output', () => {
  ...
  assert.ok(publish);
  assert.ok(publish.args.includes('homebridge-simple-ir-fan-1.2.3.tgz'));
...
test('verifies published metadata and rejects empty or mismatched verification responses', () => {
  ...
  assert.ok(readCalls(success.calls).some((call) => call.args[0] === 'view' && call.args.includes('--json')));
